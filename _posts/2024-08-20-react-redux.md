...
| 개념 | 설명 |
| Store | 애플리케이션의 전역 상태를 저장하는 공간. 오직 하나의 스토어만 존재. |
| Action | 상태에 변화를 일으키기 위해 발생하는 이벤트. type과 payload로 이루어짐. |
| Reducer | Action을 처리하여 상태를 실제로 변경하는 함수. 이전 상태와 액션을 받아 새로운 상태를 반환. |
| Dispatch | Action을 Reducer에 전달하여 상태를 업데이트하는 함수. |
| Selector | Store에서 상태를 가져오는 함수. 컴포넌트에서 필요한 상태를 선택해서 사용. |
| Action Creators | 특정 액션 객체를 생성하는 함수. |
| Action Values | 액션의 type 값을 상수로 정의하여 오타를 방지하고, 여러 곳에서 동일한 액션 타입을 사용. |
| Payload | 액션 객체에서 상태를 변경하기 위해 전달되는 데이터. |
| Middleware | 액션이 리듀서에 도달하기 전에 중간에서 특정 작업을 수행할 수 있게 해줌. |
| Ducks 패턴 | Redux 코드를 모듈화하여 관련된 액션 타입, 액션 생성자, 리듀서를 하나의 파일에 모아 관리하는 패턴. |
